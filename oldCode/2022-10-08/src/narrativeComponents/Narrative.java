...
public class Narrative {
    ...
    public void addClaim(Claim c) {
        ...
        this.claims.add(c);
        c.getNodeA().addClaim(c);
        if (!inSet(c.getNodeA())) {
            this.nodes.add(c.getNodeA());
            this.events.add(c.getNodeA());
        }
        if (c.isBinary()) {
            if (!inSet(c.getEventB())) {
                this.nodes.add(c.getEventB());
                this.events.add(c.getEventB());
            }
        } else {
            if (!c.isUnary) {
                c.getNodeB().setClaim(true);
                c.getNodeB().addEvent(c.getNodeA());
                if (!inSet(c.getNodeB())) {
                    this.nodes.add(c.getNodeB());
                    this.entities.add(c.getNodeB());
                }
            }
        }
    }
