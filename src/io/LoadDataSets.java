...
public class LoadDataSets {
    ...
    public static HashSet<DataSet> loadDataSetsFromFolder(File inputFile,String delim) {
        HashSet<DataSet> output = new HashSet<>();
        for (File entry : inputFile.listFiles()) {
            System.out.println("Load DS: " + entry);
            output.add(loadDataSetsFromFile(entry,delim));
        }
        return output;
    }

    //Loads data sets from file
    public static DataSet loadDataSetsFromFile(File inputFile,String delim) {
